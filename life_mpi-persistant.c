...
int main(int argc, char* argv[])
{
	...
	MPI_Send_init(local_board + local_ld + 1, 1, row_type, up_proc, 99, grid, &requests[0]);
	MPI_Recv_init(local_board + (2 * local_ld - 1), 1, row_type, down_proc, 99, grid, &requests[1]);

	MPI_Send_init(local_board + (2 * local_ld - 2), 1, row_type, down_proc, 99, grid, &requests[2]);
	MPI_Recv_init(local_board + local_ld, 1, row_type, up_proc, 99, grid, &requests[3]);

	// share border columns left and right
	MPI_Send_init(local_board + local_ld, local_ld, MPI_INT, left_proc, 99, grid, &requests[4]);
	MPI_Recv_init(local_board + local_ld * (col_block_size+1), local_ld, MPI_INT, right_proc, 99, grid, &requests[5]);

	MPI_Send_init(local_board + local_ld * col_block_size, local_ld, MPI_INT, right_proc, 99, grid, &requests[6]);
